...
function PlayerState_Attack_Light(){
	move = left + right;
	hsp = move*moveSpeed;
	if(vsp < 15)
		vsp+= grav;
	...
	//Start of the attack
	if(sprite_index!= sFighterLight){
		sprite_index = sFighterLight;
		image_speed = 1;
		ds_list_clear(hitByAttack);
	}
	//use attack hitbox and check for hits
	mask_index = sFighterLightHB;
	...
	if(hits > 0){
		for(var i = 0; i < hits; i++){
			...
			if(ds_list_find_index(hitByAttack,hitID) == -1){ //if we hit something that is not on the list of things that we have hit
				...
				with(hitID){
					oBottle.hp -= 5; //decrease the hp of the enemy by 5
				}
			}
		}
	}
